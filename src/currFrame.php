...
EOF;
        ...
        switch ($cmd) {
            case 'batch': // 调整运行中的进程数为 $GLOBALS['argv'][2]
                ...
                if ($batchnum > self::MAX_PROCESS_NUM) {
                    self::exit_fail(
                            'too many process,max process num:' .
    ...
    {
        $cmd = "ps -ef | grep " . $GLOBALS['argv'][0] .
                 ' | grep -v batch | grep -v start | grep -v restart | grep -v stop | grep -v grep';
        system($cmd);
        echo PHP_EOL;
    }
    ...
    {
        $arrMsg = array_merge(
                (array) file($this->processingfile, 
                        FILE_IGNORE_NEW_LINES | FILE_SKIP_EMPTY_LINES), 
                (array) file($this->completefile, 
                        FILE_IGNORE_NEW_LINES | FILE_SKIP_EMPTY_LINES));
        if (in_array($this->currMsg, $arrMsg))
            self::exit_fail('msg was handled');
        
        $arrPro = $this->getRunningProcess();
        if (isset($arrPro[$this->currMsg]))
            self::exit_fail('msg is running');
        if ($this->restart())
            return;
        $this->msg_start();
        $start = time();
     ...
     * 
     * @param int $batchnum
     *            要启动的进程个数，最终运行的进程数等于 $batchnum,这可能要启动新的进程或暂停一定数目已启动进程
     */
    ...
    public function restart ()
    {
        ...
        if (file_exists($processfile)) {
            ...
            if ($pos != self::PROCESS_END) {
                $this->msg_restart();
                $this->writeLog($this->currMsg . "     restart", $this->monitor, 
                        false);
    ...
    public function getNextMsg ()
    {
        ...
        if (empty($arrMsg) || $arrMsg[0] == self::MSG_END)
            return NULL;
    ...
    {
        $this->isend = true;
        $this->writeLog($this->currMsg . " exit,some error: {$msg}", 
                $this->monitor, false);
        exit();
    }
    ...
    public function writeLog ($msg, $type = 'process', $split = true)
    {
        ...
        if ($split) {
            $file = $this->processlogdir . '/' . $type . '.log' . $this->currMsg .
                     '_' . date('Ymd', time());
        } else {
            $file = $this->processlogdir . '/' . $type . '.log' . '_' .
                     date('Ymd', time());
        }
        $file = getcwd() . '/' . $file;
